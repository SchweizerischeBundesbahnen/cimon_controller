...
FLASHING           = {'alert': 'lselect'} # will flash the lamp between two brightnesses with the current colour, only lasts 15 seconds
...
class HueOutput():
    ...
    def treatBuild(self, job, jobStatus):
       if job in self.groups:
          ...
          mappingName = self.groups[job]
          mapping = self.mappings[mappingName]
          builds = mapping["builds"]
          lamps = mapping["lamps"]
          logger.debug("     build belongs to mapping: {}".format(mappingName))
          ...
          if mappingName not in self.states:
             self.states[mappingName] = {}
             self.states[mappingName]["status"] = status
             self.states[mappingName]["health"] = health
             self.states[mappingName]["active"] = active
          #logger.debug("     state before merge: {}".format(self.states[mappingName]))
          # consolidate for "worst" value if multiple builds contribute to one group
          if status.value > self.states[mappingName]["status"].value:
             self.states[mappingName]["status"] = status
          if health.value > self.states[mappingName]["health"].value:
             self.states[mappingName]["health"] = health
          if active:
             self.states[mappingName]["active"] = active
          #logger.debug("     state after merge: {}".format(self.states[mappingName]))
       else:
          #logger.debug("   - treating build: {}".format(job))
