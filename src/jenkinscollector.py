...
class JenkinsCollector:
    ...
    def collect(self):
        method_param = [(self.collect_job, job_name) for job_name in self.job_names] + \
                       ...
                       [(self.collect_multibranch_pipeline_standalone, multibranch_pipeline_name) for multibranch_pipeline_name in self.multibranch_pipeline_names]
    ...
    def __status_from_color_multibranch__(self, job):
        if "color" in job:
            color_status_building = job["color"].split("_") if job["color"] else (None,)
            if color_status_building[0] == "disabled":
                return JobStatus(request_status=RequestStatus.NOT_FOUND)
    ...
    def __get_cause_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "actions" in job["lastBuild"] and job["lastBuild"]["actions"] and "causes" in job["lastBuild"]["actions"][0] and job["lastBuild"]["actions"][0]["causes"]:
            return job["lastBuild"]["actions"][0]["causes"][0]["shortDescription"]

    def __get_culprits_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "culprits" in job["lastBuild"]:
            return [culprit["fullName"] for culprit in job["lastBuild"]["culprits"]]
        return []

    def __get_number_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "number" in job["lastBuild"]:
            return job["lastBuild"]["number"]

    def __get_name_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "fullDisplayName" in job["lastBuild"]:
    ...
    def __get_duration_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "duration" in job["lastBuild"]:
            return job["lastBuild"]["duration"]

    def __get_timestamp_from_job__(self, job):
        if "lastBuild" in job and job["lastBuild"] and "timestamp" in job["lastBuild"]:
    ...
    def collect_folder(self, folder_name):
        ...
        method_param = [(self.collect_multibranch_pipeline_in_folder, (folder_name, multibranch["name"])) for multibranch in folder["jobs"]]
    ...
    def map_multibranch_pipeline(self, pipeline_name, pipeline):
        ...
        for job in pipeline["jobs"]:
            status = self.__status_from_color_multibranch__(job)
            builds[(self.name,
    ...
    def __multibranch_pipeline_in_folder__(self, folder_name, multibranch_pipeline_name):
        try:
            ...
            # ignore...
            logger.exception("Error occured requesting info for pipeline in folder %s" % self.__pipeline_name__(folder_name, multibranch_pipeline_name))
...
class JenkinsClient():
    ...
    def multibranch_pipeline_in_folder(self, folder_name, multibranch_pipeline_name):
        return json.loads(self.http_client.open_and_read("/job/%s/job/%s/api/json?depth=2" % (folder_name, multibranch_pipeline_name)))
